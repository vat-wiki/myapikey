...
- **Bump the version before every push.** After a task's code changes, bump the version as a separate `chore(release): vX.Y.Z` commit, then push both together. Feature → minor bump, fix/chore → patch. Run `npm version 0.x.y --no-git-tag-version` (it updates the version field in both `package.json` and `package-lock.json`), commit, push. Only the **root** package is versioned (it's the publishable `myapikey`) — the `@myapikey/*` workspace sub-packages stay pinned at `0.1.0`. No git tags are created.
