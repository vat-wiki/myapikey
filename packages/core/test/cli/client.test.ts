...
describe("cli/client", () => {
  ...
  describe("api — proxy surface (Bearer api key)", () => {
    it("sends Bearer <apiKey> and serializes the JSON body", async () => {
      ...
      fetchMock = mockFetch([
        { match: "/openai-chat/v1/chat/completions", response: { status: 200, body: { id: "chatcmpl-1" } } },
      ]);
      const body = { model: "gpt-4o", messages: [{ role: "user", content: "hi" }] };
      const out = await client.api(ctx, "POST", "/openai-chat/v1/chat/completions", body);
      expect(out).toEqual({ id: "chatcmpl-1" });

      expect(fetchMock.calls).toHaveLength(1);
      const c = fetchMock.calls[0];
      expect(c.url).toBe("http://g:7800/openai-chat/v1/chat/completions");
    ...
    it("throws synchronously (before fetch) when ctx.apiKey is missing", async () => {
      ...
      await expect(client.api(ctx, "POST", "/openai-chat/v1/chat/completions", { x: 1 })).rejects.toThrow(
        "No API key for /openai-chat/v1, /openai-responses/v1 or /anthropic/v1",
      );
    });
  });
});
